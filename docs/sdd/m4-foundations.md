...
>
> **本版（第 4 稿）据 Codex 评审修正 D20 的一处 P1，并清理两处旧稿残片**：第 3 稿把
> requirement/screen/operation 三种结构完全不同的工件统一交给“任意 ID 文本出现位置到下一个
> ID”的通用分块算法，但现有 Design 模板会在屏幕清单与页面↔OpenAPI 映射里重复引用同一个
> `SCR-*`，OpenAPI 的 `summary`/参数等字段也可以位于 `operationId` 之前——通用文本锚点既不
> 唯一，也不覆盖完整 operation object，仍会漏报“ID 不变但内容变化”。D20 改为三种
> artifact-specific 算法：REQ 只认 canonical `### REQ-*` heading；SCR 解析唯一的屏幕清单，
> 对无法精确归属的设计正文变化保守报告全部 canonical screen；OpenAPI 用 `yaml` 解析后按
> `paths.<path>.<method>` 提取完整 operation object、以 `operationId` 为 key 做稳定结构比较。
> 同时把包结构里遗留的旧 D22“短页/硬上限”描述改成 `changed_files` 计数校验，并把 §2.5
> 仍写成“D21 待确认”的旧句改成“已接受且已同步上层文档”。
...
- **D6 — `changed.requirements` / `.screens` / `.operations`（报表字段）与"该平台是否受
  ...
  **此处不复述、也不与 D20 冲突**——第 2 稿曾在这里遗留一句"前者是 ID 集合的对称差"，
  与后来 D20 改用的 semantic diff 正面矛盾，属于修订时的疏漏，已删除，D20 是唯一权威
...
- **D19 — `ChangedPath` 必须携带 rename 的原路径，两个 reader 后端都要提供**（修复第 1 稿
  ...
  匹配）。
- **D20 — `changed.requirements`/`.screens`/`.operations`（报表字段）使用各自工件结构对应的
  semantic diff，与 §2.5 的平台布尔判定使用不同算法（第 4 稿修正通用文本分块仍会漏报的
  P1）**：第 1 稿把这三个报表字段定义成纯 ID 集合对称差（只报告新增/删除的 ID），理由是
  "避免重蹈按 REQ-ID 分块
  ...
  变了"从来不需要覆盖"没有 REQ-ID 的段落变没变"这个问题。第 2/3 稿改成“任意 ID 文本
  出现位置到下一个 ID”的通用分块，但该算法仍错误地假设三种
  工件都满足“ID 唯一出现且位于完整对象开头”：Design 模板会重复引用 `SCR-*`，OpenAPI
  的字段可以位于 `operationId` 前后，REQ 也可能在追踪表里再次被引用。**最终算法按工件
  分开定义**（§2.5/§4.5）：requirement 只认 canonical `### REQ-*` heading 并比较完整
  Markdown section；screen 解析 Design 模板 §2 的 canonical 屏幕清单行，清单外正文若有
  无法精确归属的实质变化则保守标记 base/head 全部 canonical screen；operation 用 `yaml`
  解析 OpenAPI，遍历 `paths.<path>.<method>`，按 `operationId` 比较完整 operation object。
  三者均返回 `added`/`removed`/`changed` 集合，报表字段取并集；`breaking`（D7）仅取
  operation 的 `removed` 子集。
...
| `spec.md`（且 bucket 内同一 PR 未出现 architecture.md/plan.md 变化） | 由 impact 的整篇文档 diff 结果决定：有实质内容变化 → 全部 `existing` 平台；规范化后无变化 → 均不选中（D21：这一保守解释已在第 3 稿评审中接受并同步到 implementation-plan/runbook） | 用于 `changed.requirements` 审计 |
...
"看起来更精确但可能漏判"**。**已经确认并同步上层文档的解释（D21）**：手册
...
ID 分块方案。这条解释已经在第 3 稿评审中被接受，并同步写入 `implementation-plan.md`
的 M4 小节与 runbook §9；不再是本文档悬而未决的局部解释——详见 D21。

**`changed.requirements`/`.screens`/`.operations`（报表字段）的独立算法（D20，第 4 稿
修正为 artifact-specific semantic diff）**：与上述"是否触发平台"完全独立计算，但
**不再是**纯 ID 集合对称差——第 1 稿那样定义会让"REQ-AUTH-001 的验收标准被整段重写、
ID 本身未变"这种改动完全不出现在一个名叫"变更 requirement"的字段里，这是对字段自身
承诺的语义的误读，不是安全边际。第 2/3 稿试图用一个通用文本锚点算法修复，但三种工件的
结构不同，必须分别处理：

1. **Requirements**：只把精确匹配 `^### (REQ-[A-Z0-9]+-\d+)\s*$` 的三级标题视为
   canonical declaration；section 从该标题开始，到下一个同级/更高级标题或文档末尾结束。
   追踪表、依赖描述等位置再次出现同一 `REQ-*` 只是引用，不产生第二个锚点。base/head 按 ID
   比较规范化后的完整 section，得到 `added`/`removed`/`changed`。
2. **Screens**：canonical declaration 是 Design 模板 `## 2. 屏幕清单（Screens）` 表中的
   Screen ID 行，不是全文任意 `SCR-*` 出现位置（同一 ID 会在 §8 页面↔OpenAPI 映射中再次
   出现）。先按 ID 比较 canonical 表行；再比较去除该表后的 Design 正文——正文若有实质变化，
   M4 没有足够结构把流程/a11y/平台差异精确归给某个 screen，因此把 base/head canonical
   screen ID 的并集全部加入 `changed.screens`，宁可保守多报，不能漏报。正文无变化时只报告
   表行新增/删除/修改的 ID。
3. **Operations**：用 factory 已有的 `yaml` 依赖解析 `contracts/openapi.yaml`，遍历
   `paths.<path>.<method>`（仅标准 HTTP method），要求每个 operation object 有合法且全局唯一的
   `operationId`，以该 ID 为 key 对**完整 operation object**做 key-sorted stable serialization
   后比较。这样 `summary`/参数位于 `operationId` 之前或响应位于其后都属于同一个 operation；
   不再依赖 YAML 行顺序。解析失败、重复/非法 ID 均 fail closed，不退回文本猜测。

三种算法都返回互斥的 `added`/`removed`/`changed` 集合；对应报表字段取三者并集。

**为什么这不是重新犯上面推翻的错误**：上面放弃按 REQ-ID 精细归因，是因为它**驱动平台
...
**`breaking`（D7，随 D20 的算法调整而调整,定义本身不变）**：直接取 OpenAPI semantic
diff 返回的 operation `removed` 子集（不是三者的并集）；
...
│                               #   fetchChangedFiles（D22：分页结果与 PR changed_files
│                               #   计数不一致即抛错，不再静默截断）/
│                               #   fetchBlobAtRef（404→null）；
...
D20 不再新增一个跨工件的 `diffAnchoredBlocks`。`impact.ts` 内分别实现三个小而明确的
semantic extractor/diff：`diffRequirementSections`（只认 canonical `### REQ-*` heading）、
`diffDesignScreens`（解析 canonical 屏幕清单，正文无法归属时保守标记全部 screen）、
`diffOpenApiOperations`（用 `yaml` 解析完整 operation object 并稳定结构比较）。三者共享的
只有 `normalizeText`、集合差/并等无业务语义 helper；不共享“任意 ID 文本位置就是对象边界”
这一已经被现有模板和 OpenAPI 结构证明不成立的假设。`breaking`（D7）复用
`diffOpenApiOperations` 的 `removed` 集合。上述函数只用于报表字段；`computeImpact` 驱动
平台布尔判定的整篇文档 diff（§2.5）不调用它们——两处刻意使用不同算法，见 D20。
...
- **`changed.requirements`/`.screens`/`.operations`（D20 的 artifact-specific semantic
  diff，`impact.ts` 单测）**：requirements 覆盖 canonical heading 的新增/删除/正文重写，
  并断言追踪表里重复引用同一 REQ-ID 不会产生第二个 section；head
  ...
  → 不出现在 `changed.requirements`。screens 使用现有 Design 模板 fixture：同一 SCR-ID
  同时出现在 §2 屏幕清单与 §8 页面↔OpenAPI 映射时只认清单行为 canonical declaration；
  修改清单行只报告对应 screen，修改清单外且无法归属的流程/a11y 文本则报告 base/head 全部
  canonical screen。operations 使用 OpenAPI fixture：修改位于 `operationId` **之前**的
  `summary`/参数，或位于其后的 response，都必须把该 operationId 放进
  `changed.operations`；YAML key 重排但结构等价则不报告变化；重复/非法 operationId 或 YAML
  解析失败必须 fail closed。
- **`breaking`（`impact.ts` 单测）**：base 有、head 无的 operationId → `breaking
  =true`；仅新增 operationId、或仅修改现有 operation 的其余内容（operationId
  本身不变，落入 D20 operation object 的 `changed` 分类）→ `breaking=false`（M4 窄口径的诚实边界，
  注释里说明这类情况留给 M4.5）。
...
- `sdd gate detect --repo --pr` 产出四平台布尔 + 四个 `*_paths` + `contract_changed`
  ...
  - `ChangedPath` 的 rename 条目携带 `previousPath`，两个 reader 后端均已实现
    ...
    的半成品）。
  - `changed.requirements`/`.screens`/`.operations` 用 D20 的 artifact-specific semantic
    diff 计算：REQ 认 canonical heading、SCR 认 canonical 屏幕清单且正文无法归属时保守全报、
    OpenAPI 解析完整 operation object；能检测到“ID 不变但内容重写”，也不会把重复引用或
    `operationId` 前置字段错误地切给其它对象。
