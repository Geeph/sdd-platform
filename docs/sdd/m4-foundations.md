...
- **D18 — `existing[platform]` 与 `*_paths` 用同一次存在性检查计算，两者不可能分道扬镳
  ...
  matrix 说明改为"不会被触发到"的兜底注释,不再是主要的正确性依据，见 §2.3）。（贯穿 §2.5 全部规则的统一表述，只写一次，后文引用）**：**当 M4 阶段
...
- **D22 — `detect` 必须对 GitHub API 的文件数截断 fail closed，不能停留在"低风险待决事项"
  （修复第 1 稿对已知风险的处理方式）**：第 1 稿已经知道 compare API 与 PR-files API 都有
  各自的文件数上限，却只在 §4.2/§11 记成"评估为低风险的待决事项"——评审指出这与手册"`sdd
  impact` 执行失败、输出非法或无法判定时，`detect` 必须失败"直接冲突：截断产生的是**不完整
  但形式合法**的数据，"无法判定"必须包含"看到的数据可能不完整"这种情况，不能靠"大概率
  用不到"来打发。**修复分两部分**：(1) `detectPlatforms` 自己读变更文件列表用的
  `fetchChangedFiles`（§4.5 共享 helper，源自 `gate-hygiene.ts` 现有的
  `fetchAllChangedFiles`）修正其**现有**的截断行为——现状是 `while (files.length < 1000)`
  达到上限即静默停止分页、把已读到的部分当全量返回，这本身就是一个既有的 fail-open bug
  （`checkPrHygiene` 现在也依赖这个函数，一个 >1000 变更文件的 PR 会被 hygiene 静默地只看
  一部分——这与 hygiene 自己"任一校验失败非零退出"的既有原则不符，属于顺带修复,不是
  M4 引入的新回归）；改为持续分页直到 GitHub 返回不满一页（真正读完）,若触及一个明显不
  合理的硬上限（如 10000，纯粹防止无限循环，不是业务上限）则**抛错**而不是截断返回。
...
- **D23 — 四个 reusable workflow 需要显式的工具链安装/版本锁定步骤，"M3 命令契约"不等于
  ...
  workflow YAML 里）。
- **D24 — 默认 `GITHUB_TOKEN` 能否 checkout 产品仓,提升为实现前必须验证的前置事项，设计
  对两种验证结果都保持可插拔（评审第 7 点，原 §11 待决事项 #1 的优先级提升）**：第 1 稿把
  这个假设记成"实现时验证"的待决事项，评审指出这个假设决定 D1/D2 的整个可行性——如果默认
  `GITHUB_TOKEN`（在"required workflow + `workflow_call` 嵌套一层"这个具体场景下）不能
  `actions/checkout` 一个显式指定的、不同于当前 workflow 文件来源仓库的产品仓，那四个
  reusable workflow 的 checkout 步骤（§1.2）整个不可行，需要换成 GitHub App installation
  token（类比 M2 §2.7 已经讨论过的 Factory 生产身份问题，但这次是 CI 运行时需要、不是
  Factory 建仓时需要）。**要求**：这项验证必须在 M4 代码实现**开始前**、用隔离测试 org
  完成（复用 M2/M3 已经建立的隔离 org E2E 习惯，见 §11 #1 的具体步骤），不是写完四个
  workflow 之后才发现方案不可行。**设计上的应对**：`actions/checkout` 使用的 token 来源
  在 §1.2 的 YAML 里被隔离成单一、显式的位置（`with: token: ${{ ... }}`，第 1 版可以省略
  该行、隐式使用默认 `GITHUB_TOKEN`）——若验证证明默认 token 不够，只需要在这一行插入一个
  额外的 secret（从组织级 secret 读取 GitHub App installation token），不需要改动
  `detect`、`sdd gate detect`、`computeImpact` 或任何 job 拓扑/真值表逻辑,爆炸半径严格
  限定在"四个 reusable workflow 的 checkout 步骤怎么拿到 token"这一个点上。
...
认为前者成立后者就一定成立）；`with: repository/ref` 之外是否还需要显式 `token:` 这一行，
就是验证结果唯一影响到的地方（见 §1.2 checkout 步骤旁的注释与 D24）。
    ...
    steps:
      - uses: actions/checkout@v4
        with:
          ...
          ref: ${{ inputs.head_sha }}
          # token: 省略 = 默认 GITHUB_TOKEN（D24：这一行是唯一需要在验证结果不利时
          #   改动的地方——若默认 token 不能 checkout 产品仓，换成
          #   `token: ${{ secrets.SDD_CROSS_REPO_TOKEN }}` 即可，不影响其余逻辑）
      - name: setup java (D23)
...
2. 读 PR head SHA 的 projects.yaml，用 @sdd/schemas 的 validateProjectsDocument 校验；
   ...
   任何下游判断都建立在流沙上。
3. 由 projects.yaml 构造 declared[platform] 集合与 {path, ci} 列表；对**每个**
   component（不限于这次变更涉及的）做一次 D18 的存在性检查，得到 existing[platform]
   与 *_paths[platform]（§2.3）。
4. 读 PR 变更文件路径列表，**全量分页**（GET /pulls/{pr}/files，复用 §4.5 修好截断
   bug 后的 fetchChangedFiles，见 D22——达到分页硬上限时抛错而不是静默返回部分结果）；
   对 status='renamed' 的条目一并取得 previousPath（D19）。这份列表既用于本步骤自己的
   路径分类，也会在步骤 6 原样传给 computeImpact，全程只有一份、已验证完整的变更路径
   来源（D22）。
5. 对每条变更路径分类（§2.4 的表；status='renamed' 的条目对 path 与 previousPath 各
   分类一次，D19），累积出一个"路径规则贡献的平台信号"+ 一个 needs_impact 布尔
...
// 对一条 ChangedPath 条目分类：renamed 条目同时看 path 与 previousPath（D19），
// 返回集合而非单个值，因为两者可能匹配到不同 component。
function classify(entry: ChangedPath, components: ComponentRef[]): ComponentRef[] {
  const hits = [mapPath(entry.path, components)];
  if (entry.previousPath) hits.push(mapPath(entry.previousPath, components));
  return hits.filter((c): c is ComponentRef => c !== undefined);
}

// D18：对每个 component（不限于这次变更涉及的）做一次存在性检查。
// 优先一次性读 head tree（GET /git/trees/{head_sha}?recursive=1），
// 对每个 component.path 检查是否存在 path 本身或以 `${path}/` 为前缀的条目；
// tree 返回 truncated:true 时，改为对每个 component 逐个调用
// GET /contents/{path}?ref=head_sha（200=存在、404=不存在，其余状态码 fail closed）。
async function checkExisting(
  components: ComponentRef[], reader: TreeReader,
...
**`*_paths[platform]` 的精确定义（D18）**：该平台全部 `declared` component 里，
`checkExisting` 判定为存在的那些 `path`——不是"这次 PR 改动涉及的路径"，也不是
"declared 但未检查存在性的全量列表"。`existing[platform] = *_paths[platform].length >
0`，两者由同一次检查构造，不会不同步（修复第 1 稿的 P1）。
...
`existing` 平台**，不归入"该路径看起来最像哪个平台"这类猜测,也不单独判 `detect` 失败
...
`fetchAllChangedFiles` 搬迁时一并修正其截断行为（D22）：现状
`while (files.length < 1000)` 达到上限即静默停止分页,改为持续分页直到 GitHub
返回不满一页（真正读完），仅在触及一个纯粹防死循环用途的硬上限（如 10000）时
抛错。`gate-hygiene.ts` 现有依赖这个函数的调用点不需要改代码，只是获得了一个
行为改善（此前 >1000 变更文件的 PR 会被 hygiene 静默漏看一部分，现在会正确
fail closed，见 §6）。
...
- **`fetchChangedFiles` 分页截断 fail closed（D22，`github-minimal-client.ts`
  单测，含 `gate-hygiene.ts` 现有测试套件的回归）**：mock 分页响应在达到硬上限
  时仍不满一页（模拟异常大 PR）→ 函数抛错，不返回部分结果；`checkPrHygiene`
...
- `sdd gate detect --repo --pr` 产出四平台布尔 + 四个 `*_paths` + `contract_changed`
  ...
  - `sdd impact` 失败或输出非法 → `detectPlatforms` 抛错、CLI 退出码 `3`
    （不吞错、不静默降级为全 false）；`fetchChangedFiles` 分页触及硬上限时同样
    抛错（D22），不返回部分结果。
...
5. **D21"specs/\*\* 保守全跑"解释是否需要同步写入 implementation-plan.md 或
   runbook**：这不是一个技术实现细节，是需要评审明确表态的一条解释性决定，
   详见 §0 D21、§2.5、§8 DoD 最后一条。
